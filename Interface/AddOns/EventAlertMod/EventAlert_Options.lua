...
function EAFun_SetButtonState(button, state)
	if state == 1 then
		button:SetButtonState("PUSHED", false);  	--bf@178.com
	else
...
function EventAlert_Options_Init()
	...
	-- local sTexturePath = "Interface\\AddOns\\EventAlertMod\\Images\\UI-Panel-BlueButton-Down";    --Modified by Aguan@bigfoot
...
function EAFun_GroupEvent_GetValueFromGroupItem(Param, iSpellIndex, iCheckIndex, iSubCheckIndex)
	...
	if iSubCheckIndex ~= nil then
		...
		if G_GroupItems.Spells[iSpellIndex].Checks[iCheckIndex].SubChecks and G_GroupItems.Spells[iSpellIndex].Checks[iCheckIndex].SubChecks[iSubCheckIndex] ~= nil then	--bf@178.com
...
function EAFun_GroupEvent_AddNewSpellBtn_Click(self)
	...
	if (SpellFrame == nil) then
		SpellFrame = CreateFrame("Frame", sSpellFramePrefix, EA_GroupEventSetting_Frame_SpellScrollFrameList, "BackdropTemplate");
	end
...
local function EventAlert_Options_AlertSoundSelect_Initialize()
...
function EventAlert_Options_AlertSoundSelect_OnClick(self)
	...
	--8.2, PlaySoundFile不再支援路徑形式,僅能使用FileDataID
	if (SelValue == 1) then
