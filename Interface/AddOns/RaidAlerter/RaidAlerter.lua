...
-- provisional
local UnitGroupRolesAssigned = UnitGroupRolesAssigned or function() return "" end

--初始设置
RaidAlerter_SET = RaidAlerter_SET or {
	OnOff = true,						--启用警报
...
local RaidAlerter = {};
local RAL = {};
local RaidAlerter_UINameEN = "RaidAlerter";
local RaidAlerter_Version = GetAddOnMetadata("RaidAlerter", "Version");
...
local RaidAlerter_SYNC_SELECT_TMPPRA = 0;
local RaidAlerter_SYNC_SELECT_TOME = 1;
local RaidAlerter_SYNC_SELECT_SETTOME = 0;
local RaidAlerter_SYNC_SELECT_NAME = "|cFFFF0000"..RAL_TEXT_SYNC_SELECT_NONE.."|r";
local RaidAlerter_SYNC_SELECT_NAME_MSG = 0;
local RaidAlerter_SYNC_CHECKVER = 0;
local RaidAlerter_SYNC_CHECKVERCOUNT = 0;
local RaidAlerter_SYNC_VerPOINT = 23;
local RaidAlerter_SYNC_MyStatus = "FFF";
local RaidAlerter_SYNC_MyStatus_Last = "TTT";
...
local RaidAlerter_Alerter_System	= {
	["lasticon"]	= "0",
	["maxcn"]		= 0,
	["stcn"]		= 0,
	["stcntmp"]		= 0,
...
local RaidAlerter_Alerter_dangerHealth = {};
local RaidAlerter_Alerter_BeControled = {};
local RaidAlerter_Alerter_ErrorCure = {};
for i = -4, 40 do
	local unit = "";
	if i < 0 then
		unit = "party"..abs(i);
...
function RaidAlerter_OnEvent(self, event, ...)
	if event == "ADDON_LOADED" then
		...
		if not RaidAlerter_SET.checkV then
			RaidAlerter_SET_AttackFramePosition = {"TOP","UIParent","TOP",20,-12}
			RaidAlerter_SET.checkV = true
		else
...
function RaidAlerter_SendAddonMessage()
	if not RAL_UIMSG_SENDED then
		if RaidAlerter_GetTimer("RAL_UIMSG_TIME") >= 0.5 then		--4.0后SendAddonMessage()使用频率限制?
