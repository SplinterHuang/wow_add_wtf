...
GetBankAutosortDisabled = C_Container and C_Container.GetBankAutosortDisabled or GetBankAutosortDisabled
GetBackpackAutosortDisabled = C_Container and C_Container.GetBackpackAutosortDisabled or GetBackpackAutosortDisabled
	...
	if ( systemID > 0 and not IsInventoryItemProfessionBag("player", ContainerIDToInventoryID(systemID)) ) then
		for i = LE_BAG_FILTER_FLAG_EQUIPMENT, NUM_LE_BAG_FILTER_FLAGS do
			...
			if ( systemID > NUM_BAG_SLOTS ) then
				active = GetBagSlotFlag(systemID - NUM_BAG_SLOTS, i);
			else
...
function Bag:Set(parent, id)
	...
	if id == BACKPACK_CONTAINER or id == BANK_CONTAINER then
		...
		SetItemButtonTextureVertexColor(self, 1, 1, 1)
	elseif id == Enum.BagIndex.Reagentbank then
...
function Bag:UpdateTexture()
	local id = self:GetID()
	if id > 0 or id == Enum.BagIndex.Reagentbank then
...
function Bag:UpdateTextureFromLive()
	...
	if texture then
		...
		self.hasItem = nil
		if id == Enum.BagIndex.Reagentbank then
			SetItemButtonTexture(self, 'Interface/Icons/Achievement_GuildPerk_BountifulBags')
			if IsReagentBankUnlocked() then
				SetItemButtonTextureVertexColor(self, 1, 1, 1)
			else
...
function Bag:OnClick(button)
	...
	if button == "RightButton" then
		...
		if not((link and HandleModifiedItemClick(link)) or InvData:IsCachedBag(bagID, player)) then
			...
			elseif bagID == Enum.BagIndex.Reagentbank or bagID > (GetNumBankSlots() + 4) then
				self:PurchaseSlot()
			elseif bagID > 0 then
...
function CombuctorFilterDropDown_OnLoad()
	if not g_currAPIBagID or not g_currBagID or not BAG_FILTER_LABELS then return end
			...
			if ( i ~= LE_BAG_FILTER_FLAG_JUNK ) then
				...
				info.func = function(_, _, _, value)	--value表示当前选项之前是否已被选中
					...
					if (id > NUM_BAG_SLOTS) then
						C_Container.SetBagSlotFlag(id - NUM_BAG_SLOTS, i, value);
					else
						C_Container.SetBagSlotFlag(id, i, value);
					end
				...
				if (CFDD_currBag.localFlag) then
					...
					if (id > NUM_BAG_SLOTS) then
						info.checked = GetBagSlotFlag(id - NUM_BAG_SLOTS, i);
					else
	...
	info.func = function(_, _, _, value)
		...
		elseif (id > NUM_BAG_SLOTS) then
			C_Container.SetBagSlotFlag(id - NUM_BAG_SLOTS, LE_BAG_FILTER_FLAG_IGNORE_CLEANUP, not value);
		else
			C_Container.SetBagSlotFlag(id, LE_BAG_FILTER_FLAG_IGNORE_CLEANUP, not value);
		end
	end;
	if (id == -1) then
	...
	elseif (id > NUM_BAG_SLOTS) then
		info.checked = GetBagSlotFlag(id - NUM_BAG_SLOTS, LE_BAG_FILTER_FLAG_IGNORE_CLEANUP);
	else
...
function Bag:PurchaseSlot()
	...
	if self:GetID() == Enum.BagIndex.Reagentbank then
		if not IsReagentBankUnlocked() then
