...
function lib:RequestData(what, target, force, guid)
	...
	guid = self:AddCharacter(target, guid);

	if guid then
		-- First check for cached
		if self.cache[guid].data == false or not self.cache[guid].data.items or #self.cache[guid].data.items < 1 or self.cache[guid].time == 0 or (time() - self.cache[guid].time) > self.maxAge or force then
...
function lib:SafeRequestItems(target, guid, skip)
	...
	if not self.cache[guid].inspect then
		canInspect = true;
	elseif self.cache[guid].inspect and tonumber(self.cache[guid].inspect) and self:GetAge(self.cache[guid].inspect) >= 1 then
		canInspect = true;
	end
...
function lib:AddCharacter(target,guid)
