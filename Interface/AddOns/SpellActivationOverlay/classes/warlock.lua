...
local DrainSoulHandler = {
    ...
    checkOption = function(option)
        ...
        elseif option == "spec:1" then
            ...
            local afflictionPoints = select(5, GetTalentTabInfo(1));
            local demonologyPoints = select(5, GetTalentTabInfo(2));
            local destructionPoints = select(5, GetTalentTabInfo(3));
