				...
				errorMessages = {
					...
					args = {
						...
						warning1 = {
							...
							func = function()
								if (Settings ~= nil) then
									Settings.OpenToCategory(addon.optionsCategoryId.filters)
								else
						...
						warning2 = {
							...
							func = function()
								if (Settings ~= nil) then
									Settings.OpenToCategory(addon.optionsCategoryId.allows)
								else
...
function addon:SetupOptions()
	...
	self.optionsFrames = {}
	self.optionsCategoryId = {}
	self.optionsFrames.general, self.optionsCategoryId.general = AceConfigDialog:AddToBlizOptions(MODNAME, nil, nil, "general")
	self.optionsFrames.filters, self.optionsCategoryId.filters = AceConfigDialog:AddToBlizOptions(MODNAME, L["Filter only ..."], MODNAME, "filters")
	self.optionsFrames.allows, self.optionsCategoryId.allows = AceConfigDialog:AddToBlizOptions(MODNAME, L["Allow only ..."], MODNAME, "allows")
	self.optionsFrames.output, self.optionsCategoryId.output = AceConfigDialog:AddToBlizOptions(MODNAME, L["Output"], MODNAME, "output")
	self.optionsFrames.profile, self.optionsCategoryId.profile = AceConfigDialog:AddToBlizOptions(MODNAME, L["Profiles"], MODNAME, "profile")
end
...
function addon:ShowConfig()
	if (Settings ~= nil) then
		Settings.OpenToCategory(addon.optionsCategoryId.general)
	else
