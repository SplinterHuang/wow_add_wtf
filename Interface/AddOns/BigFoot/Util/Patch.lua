...
do
	...
	local function LoaderEvents(frame, event, arg1)
		...
		if (BigFoot_SysTemSetTab['UtilsPatchVersion'] ~= patchVersion) then
			...
			-- https://bbs.nga.cn/read.php?tid=40720322
			SetCVar("UseSlug",0)								--Slug效果关闭 可能会导致卡顿

			BigFoot_SysTemSetTab['UtilsPatchVersion'] = patchVersion
		end
		if not BigFoot_SysTemSetTab.ARC and IsConfigurableAddOn("Auctionator") then
			AUCTIONATOR_CONFIG = nil
			BigFoot_SysTemSetTab.ARC = true
		end
