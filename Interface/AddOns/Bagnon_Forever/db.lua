...
function BagnonDB:BANKFRAME_OPENED()
	...
	for i = 1, GetNumBankSlots() do
		self:UpdateBag(i + NUM_BAG_SLOTS)
	end
end
...
function BagnonDB:SaveItem(bag, slot)
	...
	if texture then
		...
		if(itemID and count) then
			self.pdb[index] = format('%s,%d', itemID, count)
		else
			self.pdb[index] = itemID
		end
	else
		self.pdb[index] = nil
	end
...
function BagnonDB:BagUpdate()
	for i = BACKPACK_CONTAINER, NUM_BAG_SLOTS do
		self:SaveBag(i)
		for slot = 1, GetBagSize(i) do
			self:SaveItem(i, slot)
		end
	end
end
